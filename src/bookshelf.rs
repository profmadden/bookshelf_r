...
impl BookshelfCircuit {
    ...
    pub fn write_pl(&self, filepath: String, annotate: &Vec<String>) {
        ...
        writeln!(&mut f, "# Movable area : {}, utilization {}", self.cell_area(), self.cell_area()/cell_bb.area()).unwrap();
        for s in annotate {
    ...
    pub fn cell_bounds(&self) -> bbox::BBox {
        ...
        for c in 0..self.cells.len() {
            if !self.cells[c].terminal {
                ...
                bb.addpoint(self.cellpos[c].x + self.cells[c].w, self.cellpos[c].y + self.cells[c].h);
            }
        }
        bb
    }
