...
impl BookshelfCircuit {
	...
	for nidx in 0..num_nets {
		...
		for p in 0..nd {
		    ...
		    if let Ok((cn, sdir, colon, sdx, sdy)) = scan_fmt!(&line, " {} {} {} {} {}",
	...
	for i in self.cells.len()-10..self.cells.len() {
	    println!("Cell {} size {} x {}", self.cells[i].name, self.cells[i].w, self.cells[i].h);
	    for p in &self.cells[i].pins {
		println!("  Pin at {} {} net {}", p.dx, p.dy, self.nets[p.parentNet].name);
	    }
	}
    }
